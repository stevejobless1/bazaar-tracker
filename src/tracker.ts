...
async function runTracker() {
    ...
    console.log(`[Tracker] 📁 Database Path: ${process.env.DB_PATH || './bazaar.db'}`);
  ...
  const tick = async () => {
    ...
    console.log(`[Tracker] 🔄 Tick #${totalTicks} started...`);
    
    const data = await fetchBazaarData();
    if (!data || !data.success || !data.products) {
      console.log(`[Tracker] ⚠️ Tick #${totalTicks} skipped (API failure or no data) in ${Date.now() - startTime}ms`);
      return;
    }
    ...
    // Log heartbeat again after processing is complete
    logHeartbeat('tracker');
    console.log(`[Tracker] ✅ Tick #${totalTicks} finished in ${Date.now() - startTime}ms`);
    totalTicks++;
  };
